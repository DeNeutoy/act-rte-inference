...
class AdaptiveIAAModel(object):
    ...
    def do_inference_steps(self, cell, initial_state, premise, hypothesis, steps):
        epsilon = 0.05
        one_minus_eps = tf.constant(1.0 - epsilon, tf.float32,[self.batch_size])
        state = initial_state
        N = tf.constant(20, tf.float32,[self.batch_size])
        ...
        pred = lambda batch_mask,prob_compare,prob,\
            ...
            tf.reduce_any(
                tf.logical_and(
                    tf.less(prob_compare,one_minus_eps),
                    tf.less(counter,N)))
                # only stop if all of the batch have passed either threshold



        def inference_step(batch_mask, prob_compare,prob,counter, state, premise, hypothesis, acc_state):
            ...
            new_batch_mask = tf.logical_and(tf.less(prob + p,one_minus_eps),batch_mask)
